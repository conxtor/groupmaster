...
func (a *app) collectObservability(r *http.Request) (adminObservabilityView, error) {
	...
	for rows.Next() {
		...
		var count int64
		if err := rows.Scan(&platform, &kind, &count); err != nil { rows.Close(); return view, err }
		if item := stats[platform]; item != nil {
			if kind == "processing" { item.activeProcessing = int(count) } else if kind == "onboarding" { item.activeOnboarding = int(count) }
		}
	}
	rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&platform, &waiting, &waitSeconds); err != nil { rows.Close(); return view, err }
		if item := stats[platform]; item != nil { item.processingWaiting, item.processingWaitSeconds = waiting, waitSeconds }
	}
	rows.Close()
	...
	for rows.Next() {
		...
		var waiting, waitSeconds int64
		if err := rows.Scan(&platform, &waiting, &waitSeconds); err != nil { rows.Close(); return view, err }
		if item := stats[platform]; item != nil { item.onboardingWaiting, item.onboardingWaitSeconds = waiting, waitSeconds }
	}
	rows.Close()
	for _, platform := range []string{"whatsapp", "telegram"} {
		item := stats[platform]
		processingCapacity, onboardingCapacity := a.waPoolSize, a.waOnboardingSlots
		if platform == "telegram" { processingCapacity, onboardingCapacity = a.tgPoolSize, a.tgOnboardingSlots }
...
func scanLabelCounts(ctx context.Context, a *app, query string, target *[]observabilityLabelCount) error {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.Label, &item.Count); err != nil { return err }

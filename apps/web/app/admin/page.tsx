...
function AdminContent() {
  ...
  return <main className="shell adminPage">
    ...
    {observability && <>
      ...
      <section className="panel adminPanel"><div className="panelHead"><div><h2>MinIO</h2><p className="muted">Medienbestand im internen S3-kompatiblen Objektspeicher.</p></div><span className={observability.minio.connected ? "streamStatus ready" : "streamStatus"}>{observability.minio.connected ? "verbunden" : "nicht erreichbar"}</span></div><div className="natsStats"><div><small>Status</small><strong className={observability.minio.connected ? "metricGood" : "metricWarning"}>{observability.minio.connected ? "Verbunden" : "Nicht erreichbar"}</strong><span>{observability.minio.bucketExists ? "Bucket erreichbar" : "Bucket nicht bestätigt"}</span></div><div><small>Bucket / Endpoint</small><strong>{observability.minio.bucket || "—"}</strong><span>{observability.minio.endpoint || "—"}</span></div><div><small>Gesamtobjekte</small><strong>{observability.minio.objectCount.toLocaleString()}</strong><span>gespeicherte Medienobjekte</span></div><div><small>Gesamtspeicher</small><strong>{formatBytes(observability.minio.totalBytes)}</strong><span>{observability.minio.lastModified ? `Letztes Objekt: ${new Date(observability.minio.lastModified).toLocaleString()}` : "Noch kein Objekt erfasst"}</span></div></div><div className="minioBucketList"><div className="minioBucketHeader"><strong>Medien-Buckets</strong><span>{observability.minio.buckets.length} Buckets</span></div>{observability.minio.buckets.map((bucket) => <div className="minioBucketRow" key={bucket.bucket}><div className="minioBucketName"><strong>{bucket.bucket}</strong><small>{bucket.error || (bucket.bucketExists ? "Bucket erreichbar" : "Bucket nicht bestätigt")}</small></div><span>{bucket.objectCount.toLocaleString()} Objekte</span><span>{formatBytes(bucket.totalBytes)}</span><span>{bucket.lastModified ? new Date(bucket.lastModified).toLocaleString() : "—"}</span></div>)}</div>{observability.minio.error && <p className="streamError" style={{ margin: "0 19px 18px" }}>{observability.minio.error}</p>}</section>

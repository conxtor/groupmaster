...
function KnowledgeReassessmentCard() {
  const { locale } = useAppLocale();
  const at = (key: Parameters<typeof adminTranslate>[1], values?: Record<string, string | number>) => adminTranslate(locale, key, values);
  const [jobs, setJobs] = useState<ReassessmentJob[]>([]);
  ...
  const latest = jobs[0];
  async function loadJobs() {
    const response = await apiFetch("/api/v1/admin/ai-learning/knowledge-reassessment");
    if (!response.ok) throw new Error(at("loadFailed"));
    setJobs(await response.json() as ReassessmentJob[]);
  }
  useEffect(() => { void loadJobs().catch((value) => setError(value instanceof Error ? value.message : at("loadFailed"))); }, []);
  ...
  async function start() {
    const confirmation = replaceExisting ? at("kbReassessReplaceConfirm") : at("kbReassessConfirm");
    ...
    try {
      ...
      const body = await response.json().catch(() => null) as { error?: string } | null;
      if (!response.ok) throw new Error(response.status === 409 ? at("kbReassessConflict") : body?.error ?? at("kbReassessStart"));
      await loadJobs();
    } catch (value) { setError(value instanceof Error ? value.message : at("failedStatus")); } finally { setStarting(false); }
  }
  const progress = latest && latest.totalCount > 0 ? Math.min(100, Math.round(((latest.processedCount + latest.failedCount + latest.skippedCount) / latest.totalCount) * 100)) : 0;
  const statusLabels = { queued: at("queuedStatus"), running: at("runningStatus"), completed: at("completedStatus"), failed: at("failedStatus"), cancelled: at("cancelledStatus") };
  return <section className="panel adminPanel learningReassessment"><div className="panelHead"><div><h2>{at("kbReassessTitle")}</h2><p className="muted">{at("kbReassessHint")}</p></div><button className="primaryButton" type="button" disabled={starting || latest?.status === "queued" || latest?.status === "running"} onClick={() => void start()}>{starting ? at("kbReassessStarting") : at("kbReassessStart")}</button></div><label className="learningRebuildOption"><input type="checkbox" checked={replaceExisting} disabled={starting || latest?.status === "queued" || latest?.status === "running"} onChange={(event) => setReplaceExisting(event.target.checked)} /><span><strong>{at("kbReassessReplace")}</strong><small>{at("kbReassessReplaceHint")}</small></span></label>{error && <div className="notice">{error}</div>}{latest && <div className="learningJobStatus"><div><strong>{statusLabels[latest.status]}</strong><span>{latest.processedCount} verarbeitet · {latest.failedCount} fehlgeschlagen · {latest.skippedCount} übersprungen von {latest.totalCount}</span></div><div className="learningProgress"><i style={{ width: `${progress}%` }} /></div>{latest.error && <small className="statusLineError">{latest.error}</small>}</div>}</section>;
}
